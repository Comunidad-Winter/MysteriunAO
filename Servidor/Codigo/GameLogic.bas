...
If InMapBounds(X, Y) Then
    ...
    If FoundChar = 1 Then
        ...
        If UserList(TempCharIndex).flags.TDead > 0 And UserList(TempCharIndex).flags.TDead < 50 Then
            Stat = Stat & " <Principiante>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 50 And UserList(TempCharIndex).flags.TDead < 100 Then
            Stat = Stat & " <Teniente>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 100 And UserList(TempCharIndex).flags.TDead < 200 Then
            Stat = Stat & " <Comandante>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 200 And UserList(TempCharIndex).flags.TDead < 400 Then
            Stat = Stat & " <Guerrero Real>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 400 And UserList(TempCharIndex).flags.TDead < 600 Then
            Stat = Stat & " <Sanguinario>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 600 And UserList(TempCharIndex).flags.TDead < 700 Then
            Stat = Stat & " <Asesino a Sueldo>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 700 And UserList(TempCharIndex).flags.TDead < 800 Then
            Stat = Stat & " <Pretoriano Mortal>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 800 And UserList(TempCharIndex).flags.TDead < 900 Then
            Stat = Stat & " <Heroe Real>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 900 And UserList(TempCharIndex).flags.TDead < 1000 Then
            Stat = Stat & " <Devorador de Newbies>"
        ElseIf UserList(TempCharIndex).flags.TDead >= 1000 Then
            Stat = Stat & " <MONSTERKILL>"
        End If
