...
If UserList(UserIndex).Faccion.Jerarquia = 1 Then
    If UserList(UserIndex).Faccion.Matados(Enemigo(Fuerzas)) < 20 Then
        Call SendData(ToIndex, UserIndex, 0, Mensajes(Fuerzas, 12) & 20)
        Exit Sub
    End If
    
    If UserList(UserIndex).Faccion.Torneos < 0 Then
        Call SendData(ToIndex, UserIndex, 0, Mensajes(Fuerzas, 13) & 0)
        Exit Sub
    End If
    
    If UserList(UserIndex).Faccion.Quests < 0 Then
        Call SendData(ToIndex, UserIndex, 0, Mensajes(Fuerzas, 14) & 0)
        Exit Sub
    End If
